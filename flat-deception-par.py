...
# ---------------------------------------------------------------------------
# SCML parallel worker
# ---------------------------------------------------------------------------

def _scml_run_worker(args):
    (
        n, idx, target_community,
        flat_graph, original_communities,
        graph_multilayer, modified_graphs_dir, dataset_name,
        budget, k_path, actors_path, alias_to_node,
    ...
    random_flat_graph, edits_random = runRandomDeception(
        flat_graph, target_community, original_communities, budget
    )
    time_random = time.time() - start_time_random

    random_multilayer_graph = GraphUtils.reconstruct_multilayer_from_flat(
        random_flat_graph, graph_multilayer
    )
    random_graph_path = os.path.join(
        modified_graphs_dir, f"{dataset_name}-scml-random_c{idx}_r{n}.m"
    )
    random_multilayer_graph.to_m_file(random_graph_path)

    for file_path in [k_path, actors_path]:
        if os.path.isfile(file_path):
            dest = os.path.join(modified_graphs_dir, os.path.basename(file_path))
            subprocess.run(["cp", file_path, dest], check=True)

    print(f"[Worker c{idx} r{n}] Running SCML community detection on RANDOM graph...")
    ...
    random_communities = [set(nodes) for nodes in grouped_r.values()]

    ari_gen_random  = Utils.adjusted_random_index(original_communities, random_communities)
    ari_spec_random = Utils.adjusted_random_index_target(original_communities, random_communities, target_community)
    nmi_gen_random  = Utils.normalized_mutual_info(original_communities, random_communities)
    nmi_spec_random = Utils.normalized_mutual_info_target(original_communities, random_communities, target_community)
    ...
    print(f"[Worker c{idx} r{n}] RANDOM -> Mod: {modularity_random:.4f}, Saf: {safeness_random}, Dec: {deception_random:.4f}")

    # ---- MOD ----
    start_time_mod = time.time()
    modified_flat_graph_mod, edits_mod = Utils.applyDeception(
        'MOD', flat_graph, target_community, budget, original_communities
    )
    time_mod = time.time() - start_time_mod

    modified_multilayer_graph = GraphUtils.reconstruct_multilayer_from_flat(
        modified_flat_graph_mod, graph_multilayer
    )
    modified_graph_path = os.path.join(
        modified_graphs_dir, f"{dataset_name}-scml_c{idx}_r{n}.m"
    )
    ...
    print(f"[Worker c{idx} r{n}] Running SCML community detection on MOD graph...")
    ...
    modified_communities_mod = [set(nodes) for nodes in grouped_m.values()]

    ari_gen_mod  = Utils.adjusted_random_index(original_communities, modified_communities_mod)
    ari_spec_mod = Utils.adjusted_random_index_target(original_communities, modified_communities_mod, target_community)
    nmi_gen_mod  = Utils.normalized_mutual_info(original_communities, modified_communities_mod)
    nmi_spec_mod = Utils.normalized_mutual_info_target(original_communities, modified_communities_mod, target_community)

    modularity_after_mod = nx.community.modularity(modified_flat_graph_mod, modified_communities_mod)
    safeness_after_mod   = Safeness.computeSafeness(modified_flat_graph_mod, target_community, modified_communities_mod)
    deception_after_mod  = Utils.getDeceptionScore(modified_communities_mod, target_community, modified_flat_graph_mod)

    print(f"[Worker c{idx} r{n}] MOD -> Mod: {modularity_after_mod:.4f}, Saf: {safeness_after_mod}, Dec: {deception_after_mod:.4f}")

    # ---- SAFDEC ----
    modified_graph_path_saf = os.path.join(
        modified_graphs_dir, f"{dataset_name}-scml-saf_c{idx}_r{n}.m"
    )

    target_nodes_in_flat = [node for node in target_community if node in flat_graph]
    induced_subgraph = flat_graph.subgraph(target_nodes_in_flat)

    start_time_saf = time.time()
    modified_flat_graph_saf, edits_saf = Utils.applyDeception(
        'SAFDEC', flat_graph, target_community, budget, original_communities,
        subgraph=induced_subgraph
    )
    time_saf = time.time() - start_time_saf

    modified_multilayer_graph_saf = GraphUtils.reconstruct_multilayer_from_flat(
        modified_flat_graph_saf, graph_multilayer
    )
    modified_multilayer_graph_saf.to_m_file(modified_graph_path_saf)
    ...
    print(f"[Worker c{idx} r{n}] Running SCML community detection on SAFDEC graph...")
    ...
    modified_communities_saf = [set(nodes) for nodes in grouped_s.values()]

    ari_gen_saf  = Utils.adjusted_random_index(original_communities, modified_communities_saf)
    ari_spec_saf = Utils.adjusted_random_index_target(original_communities, modified_communities_saf, target_community)
    nmi_gen_saf  = Utils.normalized_mutual_info(original_communities, modified_communities_saf)
    nmi_spec_saf = Utils.normalized_mutual_info_target(original_communities, modified_communities_saf, target_community)

    modularity_after_saf = nx.community.modularity(modified_flat_graph_saf, modified_communities_saf)
    safeness_after_saf   = Safeness.computeSafeness(modified_flat_graph_saf, target_community, modified_communities_saf)
    deception_after_saf  = Utils.getDeceptionScore(modified_communities_saf, target_community, modified_flat_graph_saf)

    print(f"[Worker c{idx} r{n}] SAFDEC -> Mod: {modularity_after_saf:.4f}, Saf: {safeness_after_saf}, Dec: {deception_after_saf:.4f}")

    num_nodes = max(int(n) for c in original_communities for n in c)
    community_structure_original = communities_to_label_array(original_communities,      num_nodes).tolist()
    community_structure_random   = communities_to_label_array(random_communities,        num_nodes).tolist()
    community_structure_mod      = communities_to_label_array(modified_communities_mod,  num_nodes).tolist()
    community_structure_saf      = communities_to_label_array(modified_communities_saf,  num_nodes).tolist()
    ...
    result = {
        ...
        "time_mod": time_mod,
        "time_saf": time_saf,
        "edits_random": edits_random,
        "edits_mod": edits_mod,
        "edits_saf": edits_saf,
        "communities_random": [list(c) for c in random_communities],
        "communities_mod": [list(c) for c in modified_communities_mod],
        "communities_saf": [list(c) for c in modified_communities_saf],
...
def _pmm_run_worker(args):
    ...
    random_graph_path = os.path.join(
        modified_graphs_dir, f"edgelist-random_c{idx}_r{n}.csv"
    )
    ...
    df_random = pd.read_csv(random_com_path, header=None, names=["alias", "community"])
    grouped_random = defaultdict(set)
    for _, row in df_random.iterrows():
        node_id = alias_to_node.get(row["alias"])
        if node_id:
            grouped_random[row["community"]].add(node_id)
    random_communities_sets = [set(nodes) for nodes in grouped_random.values()]

    ari_gen_random  = Utils.adjusted_random_index(original_communities, random_communities_sets)
    ari_spec_random = Utils.adjusted_random_index_target(original_communities, random_communities_sets, target_community)
    nmi_gen_random  = Utils.normalized_mutual_info(original_communities, random_communities_sets)
    nmi_spec_random = Utils.normalized_mutual_info_target(original_communities, random_communities_sets, target_community)
    ...
    modified_graph_path = os.path.join(
        modified_graphs_dir, f"edgelist_c{idx}_r{n}.csv"
    )
    ...
    df_mod = pd.read_csv(modified_com_path, header=None, names=["alias", "community"])
    grouped_mod = defaultdict(set)
    for _, row in df_mod.iterrows():
        node_id = alias_to_node.get(row["alias"])
        if node_id:
            grouped_mod[row["community"]].add(node_id)
    modified_communities_list_mod = [list(nodes) for nodes in grouped_mod.values()]

    modified_clustering_mod = NodeClustering(communities=modified_communities_list_mod, graph=modified_flat_graph_mod, method_name="pmm")
    mod_communities_sets = [set(c) for c in modified_clustering_mod.communities]

    ari_gen_mod  = Utils.adjusted_random_index(original_communities, mod_communities_sets)
    ari_spec_mod = Utils.adjusted_random_index_target(original_communities, mod_communities_sets, target_community)
    nmi_gen_mod  = Utils.normalized_mutual_info(original_communities, mod_communities_sets)
    nmi_spec_mod = Utils.normalized_mutual_info_target(original_communities, mod_communities_sets, target_community)

    modularity_after_mod = nx.community.modularity(modified_flat_graph_mod, mod_communities_sets)
    safeness_after_mod   = Safeness.computeSafeness(modified_flat_graph_mod, target_community, mod_communities_sets)
    deception_after_mod  = Utils.getDeceptionScore(mod_communities_sets, target_community, modified_flat_graph_mod)
    print(f"[After] Modularity: {modularity_after_mod}, Safeness: {safeness_after_mod}, Deception: {deception_after_mod}")

    # ---- SAFDEC ----
    modified_graph_path_saf = os.path.join(
        modified_graphs_dir, f"edgelist-saf_c{idx}_r{n}.csv"
    )
    ...
    modified_multilayer_graph_saf.to_edgelist_csv(modified_graph_path_saf)
    ...
    df_saf = pd.read_csv(modified_com_path_saf, header=None, names=["alias", "community"])
    grouped_saf = defaultdict(set)
    for _, row in df_saf.iterrows():
        node_id = alias_to_node.get(row["alias"])
        if node_id:
            grouped_saf[row["community"]].add(node_id)
    modified_communities_list_saf = [list(nodes) for nodes in grouped_saf.values()]

    modified_clustering_saf = NodeClustering(communities=modified_communities_list_saf, graph=modified_flat_graph_saf, method_name="pmm")
    saf_communities_sets = [set(c) for c in modified_clustering_saf.communities]

    ari_gen_saf  = Utils.adjusted_random_index(original_communities, saf_communities_sets)
    ari_spec_saf = Utils.adjusted_random_index_target(original_communities, saf_communities_sets, target_community)
    nmi_gen_saf  = Utils.normalized_mutual_info(original_communities, saf_communities_sets)
    nmi_spec_saf = Utils.normalized_mutual_info_target(original_communities, saf_communities_sets, target_community)

    modularity_after_saf = nx.community.modularity(modified_flat_graph_saf, saf_communities_sets)
    safeness_after_saf   = Safeness.computeSafeness(modified_flat_graph_saf, target_community, saf_communities_sets)
    deception_after_saf  = Utils.getDeceptionScore(saf_communities_sets, target_community, modified_flat_graph_saf)
...
def _mpx_run_worker(args):
    ...
    random_graph_path = os.path.join(
        modified_graphs_dir, f"{dataset_name}-random_c{idx}_r{n}.mpx"
    )
    random_multilayer_graph.to_mpx(random_graph_path, node_map=node_to_alias)
    ...
    modified_multilayer_graph_mod = GraphUtils.reconstruct_multilayer_from_flat(
        modified_flat_graph_mod, graph_multilayer
    )
    modified_graph_path_mod = os.path.join(
        modified_graphs_dir, f"{dataset_name}_c{idx}_r{n}.mpx"
    )
    modified_multilayer_graph_mod.to_mpx(modified_graph_path_mod, node_map=node_to_alias)

    print(f"Running community detection on modified graph via script...")
    run_mpx_cd(modified_graph_path_mod, run_suffix_modified)
    modified_com_path_mod = find_community_file(run_suffix_modified)

    df = pd.read_csv(modified_com_path_mod, header=None, names=["alias", "layer", "community"])
    grouped = defaultdict(set)
    for _, row in df.iterrows():
        node_id = alias_to_node.get(row["alias"])
        if node_id:
    ...
    modified_clustering = NodeClustering(communities=modified_communities, graph=modified_flat_graph_mod)
    ...
    modified_graph_path_saf = os.path.join(
        modified_graphs_dir, f"{dataset_name}-saf_c{idx}_r{n}.mpx"
    )
    modified_multilayer_graph_saf.to_mpx(modified_graph_path_saf, node_map=node_to_alias)
    ...
    for _, row in df.iterrows():
        ...
        if node_id:
            grouped[row["community"]].add(node_id)
    modified_communities = [list(nodes) for nodes in grouped.values()]
    modified_clustering = NodeClustering(communities=modified_communities, graph=modified_flat_graph_saf)
...
class DeceptionEvaluator:
    ...
    def evaluate(self):
        ...
        if self.comm_algo.lower() == "scml":
            ...
            print(f"Running initial SCML community detection on original graph...")
            ...
            original_clustering = NodeClustering(
                communities=original_communities, graph=flat_graph, method_name="scml"
            )
            ...
            all_args = []
            for idx, (target_community, modularity_before, safeness_before, deception_before) in before_metrics.items():
                for n in range(self.n_run):
                    all_args.append((
                        n, idx, target_community,
                        flat_graph, original_communities,
                        graph.multilayer_graph,
                        self.modified_graphs_dir,
                        self.dataset_name,
                        self.budget,
                        k_path, actors_path,
                        alias_to_node,
            ...
            with Pool(processes=self.NUM_WORKERS) as pool:
                for run_key, result in pool.imap_unordered(_scml_run_worker, all_args):
            ...
            results = {}
            with Pool(processes=self.NUM_WORKERS) as pool:
                for run_key, result in pool.imap_unordered(_pmm_run_worker, all_args):
                    results[run_key] = result
            ...
            flat_graph = graph.get_flatten()
            original_clustering = NodeClustering(communities=original_communities, graph=flat_graph, method_name="mpx")
            original_communities = [set(c) for c in original_clustering.communities]
            ...
            results = {}
            with Pool(processes=self.NUM_WORKERS) as pool:
                for run_key, result in pool.imap_unordered(_mpx_run_worker, all_args):
                    results[run_key] = result
...
def main():
    ...
    csv_dir     = os.path.join(output_dir, f"deception_{args.comm_algo}_budget{args.budget}_Flat-DECEPTION")
